...
const doc = new MarkdownDocument()
  ...
  .code("ts", extractDeclaration("src/work/router.ts", "parseCommand"))
  .code("ts", extractDeclaration("src/work/router.ts", "routeCommand"))
  .rule()
  .heading(2, "Init Flow")
  .code("ts", extractDeclaration("src/work/init.ts", "executeInit"))
  .rule()
  .heading(2, "Next / Resume / Auto-Advance")
  .code("ts", extractDeclaration("src/work/advance.ts", "executeNext"))
  .code("ts", extractDeclaration("src/work/advance.ts", "executeResume"))
  .rule()
  .heading(2, "Config Display")
  .code("ts", extractDeclaration("src/work/config-display.ts", "executeConfig"))
  .code("ts", extractDeclaration("src/work/config-display.ts", "displayConfigOverview"))
  .toString();
