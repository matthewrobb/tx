...
const doc = new MarkdownDocument()
  ...
  .code("ts", extractDeclaration("src/execute/run.ts", "readIssuesForExecute"))
  .code("ts", extractDeclaration("src/execute/run.ts", "moveToInProgress"))
  .code("ts", extractDeclaration("src/execute/run.ts", "executeGroups"))
  .rule()
  .heading(2, "Post-Execution Delegation")
  .code("ts", extractDeclaration("src/execute/delegation.ts", "executeCodeReview"))
  .code("ts", extractDeclaration("src/execute/delegation.ts", "executeQA"))
  .code("ts", extractDeclaration("src/execute/delegation.ts", "executeShip"))
  .toString();
