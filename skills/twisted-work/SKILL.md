...
4. Suggest the best-fit preset based on detected tools per **Tool Detection**.
5. Ask user to confirm or change the suggested preset.
6. If `settings.json` exists:
   - Load and merge with **Built-in Defaults** using **Three-Layer Config Resolution**.
   - Show merged result with each value labelled `(custom)`, `(default)`, or `(preset)`.
...
7. If `settings.json` does not exist:
   - Show the merged result (defaults + selected preset).
   - Ask if the user wants to override any values.
   - Write only the `preset` field and any customized keys to `settings.json`.
