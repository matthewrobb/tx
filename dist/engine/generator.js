 ...
 */
function findNextStep(order, needsMap, completed) {
 ...
 */
export function generateMachine(workflow) {
    ...
    if (!dag.ok) {
        ...
        throw new Error(`Workflow "${workflow.id}" has invalid DAG: cycles in [${dag.cycles.map((c) => c.join(', ')).join('; ')}]`);
    }
     ...
     */
    function nextStep(completedSteps, justCompleted) {
    ...
    const machine = setup({
        ...
        guards: {
            isCurrentStep: ({ context, event }) => {
                ...
                if (!('step' in event))
                    return false;
                return isCurrentStep({ context, event: event });
            },
        },
        actions: {
