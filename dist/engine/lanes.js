 ...
 */
export function findLane(config, dir) {
 ...
 */
export function laneSequenceForType(config, epicType) {
 ...
 */
export function nextLane(config, epicType, currentLaneDir) {
 ...
 */
export function canEnterLane(lane, ctx) {
    const entryRequires = lane.entry_requires ?? [];
    if (entryRequires.length === 0)
        return true;
 ...
 * @param ctx - Predicate evaluation context.
 */
export function computeAdvancement(config, epicType, currentLaneDir, allStepsComplete, ctx) {
    if (!allStepsComplete) {
