...
export function resolveConfig(settings) {
    ...
    const mergedScalars = deepMerge(defaultRest, userRest);
 ...
 */
function mergeWorkflows(defaults, userWorkflows) {
    ...
    for (const userWf of userWorkflows) {
        if (!userWf.id)
            continue; // Skip workflows without an id
        ...
        if (base) {
            ...
            const merged = {
                ...
                steps: userSteps !== undefined ? userSteps : base.steps,
            };
            if (baseId === userWf.id) {
