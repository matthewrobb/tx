...
export async function getChildren(db, parent_id, tx) {
 ...
 */
export async function shouldAutoClose(db, parent_id, tx) {
    const children = await getChildren(db, parent_id, tx);
    if (children.length === 0) {
        return false;
    }
    return children.every((child) => child.status === 'done' || child.status === 'archived');
}
/**
 ...
 */
export async function propagateDone(db, child, tx) {
    ...
    const closedParent = await updateIssue(db, child.parent_id, { status: 'done' }, tx);
