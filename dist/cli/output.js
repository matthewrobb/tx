...
export function setCurrentCommand(name) {
 ...
 */
export function formatResponse(res, agentMode) {
    if (agentMode) {
 ...
 */
export function printResponse(res, agentMode) {
 ...
 */
export function printError(message, agentMode) {
    const errorRes = { status: 'error', message };
    process.stdout.write(formatResponse(errorRes, agentMode) + '\n');
    process.exit(1);
}
// ── Human formatting ──────────────────────────────────────────────────────────
function formatHuman(res) {
    if (res.status === 'error') {
...
function formatAction(action) {
    switch (action.type) {
        ...
        case 'run_agents': {
            const lines = action.agents.map((a) => `  [${a.model}] ${a.story}: ${a.prompt}`);
            return `Run agents:\n${lines.join('\n')}`;
        }
        case 'install_cli':
            return `Install CLI:\n${action.instructions}`;
...
function formatOkData(data) {
    ...
    if (typeof data === 'object' && !Array.isArray(data)) {
        ...
        if (typeof d['session'] === 'object' && d['session'] !== null) {
            return formatSession(d['session']);
        }
        // Checkpoint response
...
function formatIssueList(issues) {
    ...
    const rows = issues.map((i) => `${i.issue.padEnd(24)}  ${i.type.padEnd(10)}  ${i.step.padEnd(16)}  ${i.status}`);
    return [header, sep, ...rows].join('\n');
}
function formatIssueState(i) {
    const tasks = i.tasks_total !== null
...
function buildAgentResponse(res) {
