...
import type { SetupState, SetupAnswers } from './questions.js';
...
export type InitResult =
  | { status: 'prompting'; action: AgentAction; state: SetupState }
  | { status: 'complete'; config: TwistedConfig; settingsPath: string }
...
export async function runInit(input: InitInput): Promise<InitResult> {
  ...
  if (state === undefined) {
    const initial: SetupState = { step: 'welcome', answers: {} };
    return {
      status: 'prompting',
...
// ---------------------------------------------------------------------------
// Response processing — updates answers and advances step
// ---------------------------------------------------------------------------

type ProcessResult =
  | { status: 'ok'; state: SetupState }
  | { status: 'error'; message: string };
...
function processResponse(
  ...
  const answers: Partial<SetupAnswers> = { ...state.answers };
    ...
    case 'workflow_style': {
      ...
      if (style === null) {
        return {
          status: 'error',
          message:
            `Invalid workflow style "${response ?? ''}". ` +
 ...
 */
function buildSettings(answers: Partial<SetupAnswers>): TwistedSettings {
  const style = answers.workflow_style ?? 'standard';

  // Base settings depend on workflow style
  let settings: TwistedSettings;
 ...
 */
function parseWorkflowStyle(
  response: string,
): 'simple' | 'standard' | 'custom' | null {
 ...
 */
function parsePackageList(response: string): string[] {
 ...
 */
function isAffirmative(response: string | undefined): boolean {
