 ...
 */
export function createAgentSymlink(
  projectRoot: string,
  epicName: string,
  epicDir: string,
): void {
 ...
 * @param epicName - Name of the epic.
 */
export function removeAgentSymlink(projectRoot: string, epicName: string): void {
 ...
 */
export function syncAgentSymlinks(
  projectRoot: string,
  epics: Array<{ epic: string; dir: string }>,
): void {
  const dir = agentsDir(projectRoot);
  mkdirSync(dir, { recursive: true });
