...
import {
  twistedDir, ensureDir,
  readActiveSession, readCoreState, writeCoreState,
  writeNotes, writeTasks,
  findEpics, locateEpic,
  writeSettings, moveDir,
} from "../fs.js";
...
export function registerLifecycleCommands(program: Command, ctx: CliContext): void {
  ...
  program
    ...
    .action((epicName: string, opts: { reason?: string }) => {
      const location = locateEpic(root, epicName);
      if (!location) {
  ...
  program
    ...
    .argument("[epic]", "epic name")
    .action(async (epicArg: string | undefined) => {
      const globalEpic = program.opts().epic as string | undefined;
      const epicNameParam = epicArg ?? globalEpic;
