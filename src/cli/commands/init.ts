...
import type { SetupState } from '../../setup/questions.js';
 ...
 */
export function registerInitCommand(program: Command, opts: GlobalOpts): void {
  program
    .command('init')
    ...
    .action(async (cmdOpts: { response?: string; state?: string }) => {
      ...
      if (cmdOpts.state !== undefined) {
        try {
          ...
          state = JSON.parse(cmdOpts.state) as SetupState;
        } catch {
          printError(
            'Invalid --state JSON. Pass the exact value printed by the previous tx init call.',
            opts.agent,
          );
        }
      }
      ...
      switch (result.status) {
        case 'prompting': {
          ...
          process.stdout.write(
            `Re-run with: tx init --state '${JSON.stringify(result.state)}' --response "<your answer>"\n`,
          );
        ...
        case 'complete': {
          process.stdout.write(
            `Setup complete. Settings written to: ${result.settingsPath}\n`,
          );
