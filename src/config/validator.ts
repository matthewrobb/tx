...
import type { TwistedConfig, ValidConfig, WorkflowConfig } from '../types/config.js';
...
export type ConfigError =
  ...
  | { kind: 'invalid_version'; found: string }
  | { kind: 'duplicate_workflow_id'; id: string }
  | { kind: 'invalid_expression'; workflow: string; step: string; field: string; error: string }
  | { kind: 'dag_cycle'; workflow: string; cycles: string[][] }
  | { kind: 'extends_cycle'; chain: string[] }
  | { kind: 'unknown_extends'; workflow: string; extends_id: string };

export type ValidatedResult =
  | { ok: true; config: ValidConfig }
...
export function validateConfig(config: TwistedConfig): ValidatedResult {
  ...
  const workflowById = new Map<string, WorkflowConfig>();
  for (const workflow of config.workflows) {
  ...
  for (const workflow of config.workflows) {
    if (workflow.extends === undefined) continue;
  ...
  for (const workflow of config.workflows) {
    ...
    const dagResult = resolveDag(workflow.steps as StepDef[]);
  ...
  const expressionFields = ['skip_when', 'done_when', 'block_when'] as const;
  ...
  const branded = config as ValidConfig;
  Object.defineProperty(branded, '_brand', {
    value: 'ValidConfig' as const,
    writable: false,
    enumerable: false,
    configurable: false,
  });
