 ...
 */
export function resolveDag(steps: StepDef[]): DagResult {
  // --- Validation ---
  ...
  const unknownDeps: Array<{ step: string; unknown: string }> = [];
  for (const step of steps) {
...
function traceCycles(
  ...
  for (const startNode of cycleNodes) {
    ...
    while (true) {
      ...
      const nextDep = step.needs.find(
        (dep) => cycleNodes.has(dep) && !visited.has(dep),
      );
