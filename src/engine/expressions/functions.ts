 ...
 */
export function createBuiltinFunctions(): Map<string, ExprFn> {
  const fns = new Map<string, ExprFn>();
  ...
  fns.set('defined', (...args: Json[]): EvalResult => {
    if (args.length !== 1) {
  ...
  fns.set('not_empty', (...args: Json[]): EvalResult => {
    if (args.length !== 1) {
      return { ok: false, error: `not_empty() expects 1 argument, got ${args.length}` };
    }
    const val = args[0];
    if (val === null) return { ok: true, value: false };
    if (typeof val === 'string') return { ok: true, value: val.length > 0 };
    if (Array.isArray(val)) return { ok: true, value: val.length > 0 };
    if (typeof val === 'object') return { ok: true, value: Object.keys(val).length > 0 };
  ...
  fns.set('includes', (...args: Json[]): EvalResult => {
    if (args.length !== 2) {
  ...
  fns.set('count', (...args: Json[]): EvalResult => {
    if (args.length !== 1) {
      return { ok: false, error: `count() expects 1 argument, got ${args.length}` };
    }
    const val = args[0];
    if (!Array.isArray(val)) return { ok: true, value: null };
