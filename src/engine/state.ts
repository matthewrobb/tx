...
/** DB row shape for the vars table. */
interface VarRow {
  key: string;
  value: Json;
}

/** DB row shape for task completion queries. */
interface TaskDoneRow {
  done: number; // PGLite stores boolean as INTEGER 0/1
}

/** Convert a WorkflowConfig (from DEFAULT_CONFIG) to the Workflow type that the engine expects. */
function toWorkflow(wc: WorkflowConfig): Workflow {
  return {
    id: wc.id as WorkflowId,
    slug: wc.id,
...
function toIssueState(issue: Issue, tasksDone: number, tasksTotal: number | null): IssueState {
  return {
    issue: issue.slug,
    type: issue.type,
...
async function loadVars(
  ...
  const result = await db.query<VarRow>(
    `SELECT key, value FROM vars WHERE issue_slug = $1 AND step = $2`,
    [issueSlug, step],
    tx,
  );
...
async function loadTasks(
  ...
  const result = await db.query<TaskDoneRow>(
    `SELECT done FROM tasks WHERE issue_slug = $1`,
    [issueSlug],
    tx,
  );
 ...
 */
function validateWithMachine(
  workflow: Workflow,
  evaluations: StepEvaluation[],
  currentStep: string,
  event: 'STEP_DONE' | 'STEP_SKIP' | 'STEP_BLOCK',
): boolean {
  try {
...
export async function txNext(
  db: StoragePort,
  projection: ProjectionPort,
  input: TxNextInput,
): Promise<TxNextResult> {
  // Steps 1–5 run inside a single DB transaction.
  const result = await db.transaction(async (tx): Promise<TxNextResult> => {
    ...
    if (input.resume_response !== undefined) {
      await db.query(
        ...
        [input.issue_slug, issue.step, JSON.stringify(input.resume_response)],
        tx,
      );
    }
    ...
    if (currentStepEval !== undefined &&
      ...
      const event = currentStepEval.resolution === 'done' ? 'STEP_DONE' as const : 'STEP_SKIP' as const;
