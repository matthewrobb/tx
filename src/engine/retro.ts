 ...
 */
export function aggregateRetro(epicDir: string, epicName: string): {
  retro: RetroNote[];
  candidates: BacklogCandidate[];
  retroMd: string;
} {
  const notesPath = join(epicDir, "notes.json");
  const notes: Note[] = existsSync(notesPath)
...
export function writeRetro(
  epicDir: string,
  epicName: string,
): { retroMd: string; candidates: BacklogCandidate[] } {
  ...
  if (candidates.length > 0) {
    writeFileSync(
      join(epicDir, "backlog-candidates.json"),
 ...
 */
export function readCandidates(epicDir: string): BacklogCandidate[] {
 ...
 */
export function promoteCandidateById(
  epicDir: string,
  candidateId: string,
): BacklogCandidate[] {
  const candidates = readCandidates(epicDir);
  const updated = candidates.map((c) =>
    c.id === candidateId ? { ...c, promoted: true } : c,
  );
  writeFileSync(
    join(epicDir, "backlog-candidates.json"),
